...
class Grid
  ...
  def desenha_grid
    #Lopping que realiza o desenho das linhas verticais do grid. As linhas começam do topo e vão até a parte inferior da janela, todas mantendo uma distância definida anteriormente.
    (Window.width / TAMANHO_QUADRADO).times do |x|
      ...
      #Lopping que realiza o desenho das linhas horizontais do grid. Essas linhas começam do inicio da janela definida e vão até o seu final, todas mantendo uma distância definida anteriormente.
      (Window.height / TAMANHO_QUADRADO).times do |y|
    ...
    def marcar(x, y)
      #Se o valor fornecido estiver presente para alguma chave em hash, então esses dados são apagados. Ou seja, se o jogador apertar em uma célula que já estava viva, ela morre
      if @grid.has_key?([x, y])
        @grid.delete([x, y])
      ...
      if @playing
        ...
        (Window.width / TAMANHO_QUADRADO).times do |x|
          (Window.height / TAMANHO_QUADRADO).times do |y|
            ...
            #Se a célula selecionada está viva e possui 2 ou 3 vizinhanças vivas ou se a célula selecionada não está viva e possui exatamente 3 vizinhanças vivas, iremos fazer uma nova entrada no novo grid 
